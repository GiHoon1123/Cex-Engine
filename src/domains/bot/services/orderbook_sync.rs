...
use crate::shared::clients::java_api::JavaApiClient;
...
pub struct OrderbookSync {
    ...
    /// 주문 서비스 (Rust 엔진 내부용, 봇은 Java API 사용)
    order_service: OrderService,
    
    /// Java API 클라이언트 (봇 주문 생성용)
...
impl OrderbookSync {
    ...
    pub fn new(
        ...
        // Java API 클라이언트 생성 (봇 주문도 Java를 통해 생성)
        let java_api_client = JavaApiClient::new(None)
            .expect("Failed to create Java API client");
    ...
    async fn create_bot_order_internal(
        ...
        let request = CreateOrderRequest {
            order_id: None, // 봇 주문은 Rust 엔진에서 직접 생성하므로 order_id 없음
            user_id, // user_id 추가
